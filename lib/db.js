...
/** One-time style: move M/T, Redeem, APS from percent to tier slabs if still on percent. */
async function migratePercentRulesToTierSlabs(db) {
  await db.execute(
    `UPDATE commission_rules
     SET calc_type = 'slab', rate = NULL, slab_base = 1000, slab_value = 10, base_field = 'transfer_amount'
     WHERE rule_key = 'money_transfer_mt' AND calc_type = 'percent'`
  );
  await db.execute(
    `UPDATE commission_rules
     SET calc_type = 'slab', rate = NULL, slab_base = 100, slab_value = 10, base_field = 'amount'
     WHERE rule_key = 'money_transfer_redem' AND calc_type = 'percent'`
  );
  await db.execute(
    `UPDATE commission_rules
     SET calc_type = 'slab', rate = NULL, slab_base = 1000, slab_value = 10, base_field = 'amount'
     WHERE rule_key = 'money_transfer_aps' AND calc_type = 'percent'`
  );
}
...
async function seedCommissionRules(db) {
  ...
  const defaults = [
    ...
    {
      rule_key: "money_transfer_aps",
      label: "APS (Aadhaar Payment)",
      calc_type: "slab",
      rate: null,
      slab_base: 1000,
      slab_value: 10,
...
export async function ensureLeadsTable() {
  ...
  try {
    ...
    await migratePercentRulesToTierSlabs(db);
    try {
