...
import { adminTodayIso, titleCase } from "@/lib/format";
...
/** List/card total for commission types = txn face value + commission; else txn/sale amount. */
export function entryCollectionAmount(entry) {
  const txn = entryTxnAmount(entry);
  if (entry?.category === "payment") return txn;
  if (commissionEligible(entry?.category, entry?.mt_subtype)) {
    return txn + entryRevenueAmount(entry);
  }
  return txn;
}
...
export function computeTotals(entries) {
  ...
  /** Transaction volume + commission (recharge/M/T) + sales + payment in. */
...
/** Category card total = txn + profit for commission types; else sale amount. */
export function categoryCollectionTotal(totals, category) {
  if (!totals) return 0;
  switch (category) {
    case "recharge":
      return (totals.total_recharge_gross ?? 0) + (totals.total_recharge ?? 0);
    case "money_transfer":
      return (totals.total_mt_gross ?? 0) + (totals.total_mt ?? 0);
    case "accessory":
